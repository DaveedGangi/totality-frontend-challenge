    ...
    render(){
      ...
      <div className="NavBarForRental">
        ...
        <button className="LogOutButton" type="button" onClick={this.removeCookie}>Log out</button>
                               ...
                               {
                                cartList.map((each)=>(
                                    <div key={each.storageForRentalid}>
                                        <img key={each.id} className="CartImageItem" src={each.imageUrl} alt={each.title}/>
                                      <h1 className="cartImageTitle">{each.title}</h1>
                                      
                                       <h1>Rs {each.pricess} L</h1>
                                       <button key={each.id} id={each.id} onClick={removing} type="button">Delete</button>
                                    </div>
                                ))
                               }
                            </div>
