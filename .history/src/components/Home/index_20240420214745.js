...
class Home extends Component {
  ...
  state={storageForRental:[],rentalDataStatus:rentalState.initial,duplicateRentalStorage:[],pageNumber:1,storageOfItems:[]}
  ...
  counting=(event)=>{
    const{storageOfItems}=this.state 
    const idForItem=event.target.id
    this.setState(prevState=>({storageOfItems:[...prevState.storageOfItems,idForItem]}))
    console.log(event.target.id)
    const updateStorageOfItems=storageOfItems.filter((each)=>console.log(each))
    
   
    this.setState({storageOfItems:updateStorageOfItems})
...
render(){
  ...
  const{storageForRental,pageNumber,storageOfItems} = this.state
    ...
    return (
     <div>
      ...
      <div className="NavBarForRental">
        ...
        <p><BsCartDash />&nbsp;{storageOfItems.length}</p> 
        </Link>
        </div>
